...
export class SelectFormLoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.applyForm
      .get('escola')
      ?.valueChanges.subscribe(this.onChangeSchool.bind(this));
    this.applyForm
      .get('serie')
      ?.valueChanges.subscribe(this.onChangeSerie.bind(this));
    this.applyForm
      .get('turma')
      ?.valueChanges.subscribe(this.onChangeTurma.bind(this));
    this.applyForm
      .get('formulario')
  ...
  onChangeSchool(value: any) {
    ...
    this.serieOptions = [
      ...new Set<string>(value['turmas'].map((turma: any) => turma.serie)),
    ];
  ...
  onChangeSerie(value: any) {
    ...
    if (this.applyForm.value['escola'] && value) {
      this.turmasOptions = this.applyForm.value['escola'].turmas.filter(
        (turma: any) =>
          getAnoFromSerieString(turma.serie) == getAnoFromSerieString(value)
      );
        ...
        next: (response) => {
          ...
          // trocar por uma dialog de alerta/ fazer componentes de alerta
            ...
            const formsOfTurma = response
              ...
              .map(
                (form: any): UserDataLocalStorage =>
                  this.passToUserDataLocalStorage(form)
              );
  ...
  passToUserDataLocalStorage(data: any): UserDataLocalStorage {
    ...
    data['turma'] = this.applyForm.value['escola'].turmas.find(
      (turma: any) => turma['_id'] === data['turma']
    );
