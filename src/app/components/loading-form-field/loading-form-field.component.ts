...
import { EventEmitter, Component, Input, Output, ViewEncapsulation } from '@angular/core';
  ...
  ngOnInit(){
    this.loadingData = this.fetchData?.pipe(
      catchError((error) => {
        ...
        console.error(`Erro ao carregar entidade '${this.label.toLocaleLowerCase()}':`, error);
        return of([]); 
      })
    );
