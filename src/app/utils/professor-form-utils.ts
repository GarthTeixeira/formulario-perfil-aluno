...
export function formatarTelefone(event: any, formulario: AbstractControl<any> | null): void {
      ...
      if (rawValue.length > 2) {
        ...
        if (rawValue.length > 6) {
          ...
          if(rawValue.length >= 11){
            value = `(${rawValue.substring(0, 2)}) ${rawValue.substring(2, 7)}-${rawValue.substring(7, 11)}`
            }
        }
      }
    }
    ...
    input.value = value;
    formulario?.get('telefone')?.setValue(value ? value.replace(/\D/g, '') : null);
  }